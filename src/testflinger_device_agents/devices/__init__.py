    ...
    def start(self):
        ...
        def reconnector():
            ...
            while True:
                ...
                testflinger_device_agents.logmsg(
...
class DefaultDevice:
    def runtest(self, args):
        ...
        testflinger_device_agents.logmsg(logging.INFO, "BEGIN testrun")

        test_opportunity = testflinger_device_agents.get_test_opportunity(
            args.job_data
        )
        ...
        serial_proc.start()
        try:
            exitcode = testflinger_device_agents.run_test_cmds(
                test_cmds, config
            )
            ...
            serial_proc.stop()
        testflinger_device_agents.logmsg(logging.INFO, "END testrun")
        return exitcode
    ...
    def reserve(self, args):
        ...
        with open(args.config) as configfile:
            config = yaml.safe_load(configfile)
        testflinger_device_agents.configure_logging(config)
        testflinger_device_agents.logmsg(logging.INFO, "BEGIN reservation")
        job_data = testflinger_device_agents.get_test_opportunity(
            args.job_data
        )
        try:
