...
from testflinger_device_agents.devices import ProvisioningError, RecoveryError
...
class Dragonboard:
    ...
    """Testflinger Device Agent for Dragonboard."""
    ...
    def setboot(self, mode):
        ...
        This method sets the testflinger boot method to the specified value.
        """
        if mode == "master":
    ...
    def is_test_image_booted(self):
        ...
        try:
            ...
            return False
        # If we get here, then the above command proved we are in testflinger
        return True
        ...
        try:
            self._run_control(
                "sudo mount {} /mnt".format(
                    self.config["testflinger_writable_partition"]
                )
            )
        except subprocess.CalledProcessError as exc:
            err = (
                ...
                "output: {}".format(
                    self.config["testflinger_writable_partition"], exc.output
                )
            )
    ...
    def provision(self):
        ...
        testflinger_device_agents.download(url, "testflinger.img")
        image_file = testflinger_device_agents.compress_file("testflinger.img")
        server_ip = testflinger_device_agents.get_local_ip_addr()
        serve_q = multiprocessing.Queue()
        file_server = multiprocessing.Process(
            target=testflinger_device_agents.serve_file,
            args=(
                serve_q,
                image_file,
            ),
